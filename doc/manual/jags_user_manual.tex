...
that is useful for post-processing. A monitor in \JAGS\ is defined by
three components
\begin{itemize}
\item A node or set of {\bf nodes} to be monitored
\item A statistic, or {\bf stat} to be calculated from the current state of the chain
...
The stat type 
The default stat type is ``value''
...
the current value of an array defined by a subset FIXME
...
Table~\ref{tab:density_stats} summarizes the monitor stats provided
...
\item Weighted stats. These are variants of the vector-valued stats
  ...
  diagnostics. See~\ref{section:loo} for more detailes.
\end{enumerate}
...
how to calculate these in \JAGS.
...
\caption{\label{table:fit_measures} Measures of model fit and how to calculate them in \JAGS.
  The last column (post-processing) shows how to calculate the finale statistic in R. Here
...
of these properties hold for the Bayesian deviance. In fact the
Bayesian deviance is a relative measure of model adequacy. It is
...
\citet{watanabe:2010} defined the Gibbs training loss as the empirical
mean of minus the log density over all observations
\[
...
\citet{watanabe:2010} defined the Bayes training loss as the empirical
mean of minus the log predictive density over all observations
\[
BL_t = - \frac{1}{n} \sum_{i=1}^n \log q(y_i \mid y_{1:n})
\]
%The Bayes loss is the basis of the Widely Applicable Information Criterion (WAIC)
%\citep{watanabe:2010}.

\subsubsection{Log pointwise predictive density}

\citet{gelman:etal:understanding} popularized the use of the Widely
...
\citet{spiegelhalter:dic} introduced the quantity $p_D$ as the
...
\]
They called $p_D$ the effective number of parameters. For simple
models with weak prior information $pD_$ tends to $k$, the number of
...
To overcome these issues \citet{plummer} suggested an alternative
definition. 

\subsection{WAIC penalty: $p_W$}

\citet{watanabe:book} introduced the quantity $p_W$ as an estimate
of model dimension
\[
...
called the singular fluctuation. \citet{watanabe:book} also proposed an
alternative estimate of the singular fluctuation based on the difference
between Gibbs and Bayes training loss. This leads to the alternative quantity
\[
p^*_W = 2n \left(BL_t - GL_t)
\]
Although $p_W$ and $p^*_W$ are asymptotically equivalent, empirical
investigations suggest that $p^*_W$ performs less well
\citep{gelman:understanding}.

\subsection{Variance penalty: $p_V$}

Motivated by the previously mentioned problems with $p_D$,
\cite{gelman book} proposed alternative definition of the effective
...
sources of information about the parameters \citep{plummer:2025}.
...
\begin{tabular}{lllr}
  ...
  \hline
\end{tabular}
\caption{\label{table:xomplexity_measures} Measures of model
  complexity and how to calculate them in \JAGS.  The last column
  (post-processing) shows how to calculate the finale statistic in
...
\citet{spiegelhalter:dic} introduced the Deviance Information Criterion (DIC) as
\begin{align*}
  \text{DIC} & = \overline{D} + p_D \\
             & = D(\overline{\theta}) + 2 p_D
\end{align*}
In the second form DIC can be seen as a Bayesian generalization of the
Akaike Information Criterion (AIC) \citep{akaike}.
\[
...
\citet{spiegelhalter:dic}, we still refer to this as DIC, since
...
\citet{watanabe:book} derived the Widely Applicable Information
...
\citet{gelman:understanding} popularized WAIC using terminology more
...
any value of the outcome variable $y$. A theory of Bayesian leverage
is outlined by \citet{plummer2025}.
...
test data with high leverage indicates {\em covariate shift} and the
predictions for that observation should be considered unreliable
...
\citep{plummer:2025}. This means you must run at least two chains when
...
Stat types \texttt{leverage\_total} and \texttt{loo\_leverage} are
discussed in sections~\ref{section:dic} and \ref{section:loo}
respectively.
