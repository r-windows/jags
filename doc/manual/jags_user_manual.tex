...
on the current state of the Markov chain and summarizes them in a
way that is useful for post-processing of the MCMC output. A monitor in
\JAGS\ is defined by three components
\begin{itemize}
\item A set of {\bf nodes} to be monitored
...
{\bf type} was split into {\bf stat} and {\bf summary} to make it
easier to define a wider range of monitors.
...
Suppose $d$ is the size of the stat being monitored, $n$ is the number
of iterations and $t$ is the thinning interval.
...
Summary type \texttt{mean} records a running mean of the stat in a
vector of size $d$. The size of the summary is independent of the
number iterations $n$ or thinning interval $t$. If you are only
interested in the posterior mean of the stat then \texttt{mean} is a
much more memory-efficient sumamry than \texttt{trace}. This may be
important when the size of the stat $d$ is very large.

\subsection{Variance}

Summary type \text{var} records the running variance of each element
of the stat in a vector of size $d$. It may be be used in conjunction
with another sampler with summary type \texttt{mean} to provide
estimates of the posterior mean and variance. If the stat has a
posterior normal distribution then these two monitors provide
sufficient statistics for the marginal posterior of each element of
the stat.
...
The \texttt{diag} module provides
