...
\section{Node density stats}

Most of the monitor stats in the \texttt{diag} module are functions on
the probability density function for stochastic nodes. Some notation
is required to define this. Suppose that $y_1 \ldots y_n$ are the
values of the observed nodes and that the nodes are {\em a priori}
...
zero. This is done to allow density statistics to be aggregated over
node arrays that contain both stochastic and non stochastic nodes by
summing the log densities, so that the non-stochastic nodes make no
...
of the leverage stat over all iterations. The recommended summary type
to use with stat type \texttt{leverage} is therefore
...
There are two commonly used measures of model fit, the expected log-likelihood,
and the log predictive density. The expected log likelihood for observation $i$
is
\[
\text{E}_{\theta} \left( \log p(y_i \mid \theta) \mid y_{1:n}\right)
\]
The log predictive density is
\[
\log \text{E}_{\theta} \left( p(y_i \mid \theta) \mid y_{1:n} \right)
\]
