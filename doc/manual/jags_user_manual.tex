...
distribution modifier on the right hand side of a stochastic
relation. If
\begin{verbatim}
X ~ dfoo(theta) T(L,U)
\end{verbatim}
...
JAGS also supports an alternative truncation syntax from NIMBLE, in
which \texttt{T()} is a wrapper around the distribution that adds
additional arguments for the lower and upper bound:
\begin{verbatim}
X ~ T(dfoo(theta), L, U)
\end{verbatim}
