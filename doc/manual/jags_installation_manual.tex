...
\Rtools\ comes with its own version of MSYS, a minimal unix shell
that we will use to build \JAGS. At the time of writing, the MSYS
shell does not include the \Rtools\ compiler tools on the PATH,
...
\Rtools\ contains static libraries for BLAS and LAPACK. In order
...
\texttt{pkg-config} is not installed automatically with \Rtools. You
can install it with
\begin{verbatim}
