...
The instructions below are based on Linux. They may also be applicable
to UNIX but have not been tested on other platforms.

\JAGS\ follows the usual convention of 
\begin{verbatim}
./configure
make
make install
...
\option{-j} to speed up compilation, {\em e.g.} to use 8 cores you
may use:
\begin{verbatim}
make -j8
\end{verbatim}
...
practice is to build \JAGS\ on a platform without an existing
installation.  Otherwise, run the tests after \code{make install} so
the build and installed versions are the same.
...
installed in \file{lib64}, whereas on Debian, and other Linux
distributions that conform to the Filesystem Hierarchy Standard (FHS),
the correct installation directory is \file{lib}.
...
\code{LD\_LIBRARY\_PATH} to include \file{\$JAGS\_HOME/lib}.

\subsection{BLAS and LAPACK}
\label{section:blas:lapack}

BLAS (Basic Linear Algebra System) and LAPACK (Linear Algebra Pack)
are two libraries used by \JAGS\ for dense linear algebra.  There now
several competing implementations of BLAS and LAPACK that can be
linked against \JAGS.

These are the open source implementations that are likely to be available
for a Linux distribution:
\begin{itemize}
\item The {\bf BLAS} and {\bf LAPACK} libraries from Netlib
  (\url{https://www.netlib.org}) are the reference implementations.
\item {\bf OpenBLAS} is an optimized BLAS library that may be used as
  a replacement for Netlib BLAS.
\item {\bf ATLAS} (Automatically Tuned Linear Algebra System) is an
  optimized linear algebra library that implements all of BLAS and
  most of LAPACK.
\item {\bf BLIS} is a portable framework for instantiating BLAS-like
  libraries for dense linear algebra operations. While BLIS has its
  own API it also includes a BLAS compatibility layer and can be used
  as a drop-in replacement for BLAS. The same team that created BLIS
  also created {\bf libflame}, which provides much of the
  functionality of LAPACK.
\end{itemize}
...
In all cases, if your Linux distribution splits packages into ``user''
and ``developer'' versions, then you must install the developer
package with suffix \texttt{-devel} (for RedHat derivatives) or
\texttt{-dev} (for Debian derivatives).

\subsubsection{Choosing BLAS and LAPACK at configure time}

During configuration, a default list of possible BLAS and LAPACK
implementations will be checked. If \texttt{configure} cannot find a
suitable library, it will stop with an error message.

You may manually choose the BLAS and LAPACK libraries using the
configure options \texttt{--with-blas} and \texttt{--with-lapack}
\begin{verbatim}
./configure --with-blas="-lmyblas" --with-lapack="-lmylapack"
\end{verbatim}

If the BLAS and LAPACK libraries are in a directory that is not on the
default linker path, you must set the \code{LDFLAGS} and
\code{LD\_LIBRARY\_PATH} environment variables to point to this
...
You also need to set \code{LD\_LIBRARY\_PATH} when you want to run
\JAGS.  Alternatively, you may be able to hard-code the paths to the
blas and lapack libraries at compile time. This is compiler and
platform-specific, but is typically achieved with
\begin{verbatim}
LDFLAGS="-L/path/to/my/libs -R/path/to/my/libs"
\end{verbatim}

\subsubsection{Using FlexiBLAS to choose BLAS/LAPACK at run time}
...
\end{verbatim}
If you are using an R interface to \texttt{JAGS}, you can use the
functions in the CRAN package \texttt{flexiblas} to set the backend
and, for multithreaded backends, to control the number of threads.

\subsubsection{Using alternatives to choose BLAS/LAPACK at run time}

The alternatives mechanism on Debian and Ubuntu is an operating system
feature that allows you to change (among other things) the working backend
for BLAS/LAPACK without recompiling \JAGS.

To use the alternatives mechanism you must explicitly link \JAGS\ to
\code{libblas} and \code{liblapack}.
\begin{verbatim}
./configure --with-blas="-lblas" --with-lapack="-llapack"
\end{verbatim}
The configure script will not do this by default because it assumes
that \code{libblas} and \code{liblapack} are the default Netlib
implementations, which are inefficient compared to other options. In
fact, on Debian and Ubuntu these libraries are symbolic links to
backend libraries that can be modified using the
\code{update-alternatives} command line interface.

First see which version BLAS and LAPACK the \JAGS\ library is actually
linked against, {\em e.g.}:
\begin{verbatim}
$ ldd /usr/local/lib/libjags.so | grep -e "libblas" -e "liblapack"
	liblapack.so.3 => /lib/x86_64-linux-gnu/liblapack.so.3 (0x000071625d000000)
	libblas.so.3 => /lib/x86_64-linux-gnu/libblas.so.3 (0x000071625cf93000)
\end{verbatim}
Then see the list of alternatives available
\begin{verbatim}
update-alternatives --display liblapack.so.3-x86_64-linux-gnu
\end{verbatim}
Or, as super-user, interactively choose the backend from a menu
\begin{verbatim}
update-alternatives --config liblapack.so.3-x86_64-linux-gnu
\end{verbatim}
You can choose manually, or use auto mode (the default) so that the most efficient
library will be linked among currently installed alternatives.
...
Intel oneAPI includes a shell script that sets up the environment variables
...
After calling the \code{setvars.sh} script, you can link \JAGS\ with a
sequential version of MKL as follows:
\begin{verbatim}
./configure \
...
\subsection{Alternative compilers}

When building \JAGS\ with GNU compilers (\code{gcc}, \code{g++},
\code{gfortran}), the configure script automatically adds the
optimization flag ``-O2'' to make the compiled code run faster. It
will not do this for other compilers.  Therefore, when building
\JAGS\ with alternative compilers you must add the appropriate
optimization flags via the environment variables \code{CFLAGS},
\code{CXXFLAGS}, \code{FCFLAGS}, and \code{FFLAGS}. Suitable values
are given below for Intel and clang compilers. For other compilers
...
\file{setvars.sh} shell script, {\em e.g.}
\begin{verbatim}
source /opt/intel/oneapi/setvars.sh
\end{verbatim}
...
CFLAGS="-O2" CXXFLAGS="-O2", FFLAGS="-02" FCFLAGS="-O2" \
./configure 
\end{verbatim}
