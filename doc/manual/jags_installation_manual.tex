...
default linker path, you must set the \code{LDFLAGS} \nd
\code{LD\_LIBRARY_PATH} environment variables to point to this
...
BLAS and LAPACK define the standard API for dense linear algebra
operations. There are now several competing implementations of BLAS
and LAPACK that can be linked against \JAGS. Previous versions of the
\JAGS\ manual gave explicit instructions on how to link to these
alternatives. We no longer do this because most Linux distributions
provide a mechanism for changing the BLAS and LAPACK implementations
dynamically at runtime (See below). We make an exception for AOCL and
MKL which are two commercially libraries that are not included in
Linux distributions by default.

\begin{itemize}
\item The {\bf BLAS} and {\bf LAPACK} libraries from Netlib
  (\url{https://www.netlib.org}) should be provided as part of your
  Linux distribution.
...
\subsubsection{Alternatives mechanism}

The alternatives mechanism on Debian and Ubuntu is an operating system
layer that allows you to change (among other things) the working backend
