...
static double d_n_cot(double x, int n)
{
	...
	return -1/JR_pow_di(sin(x), 2);
    else if (n == 2) // 2 cos / sin^3
	return 2*cos(x)/JR_pow_di(sin(x), 3);
    else if (n == 3) { // (-4 cos^2 - 2) / sin^4 ; num.= -2(2 cos^2 +1) = -2(3 - 2 sin^2)
	// tt = -2*(2*JR_pow_di(cos(x), 2) + 1.)/JR_pow_di(sin(x), 4);
	double sin2 = JR_pow_di(sin(x), 2); // = sin^2
	return -2*(3 - 2*sin2)/JR_pow_di(sin2, 2);
    }
    else if (n == 4) { // 8 cos (cos^2  + 2) / sin^5
	double co = cos(x);
	return 8*co*(JR_pow_di(co, 2) + 2) / JR_pow_di(sin(x), 5);
    }
    else if (n == 5) { // (-16 cos^4 - 88 cos^2 - 16)/ sin^6
	double co2 = JR_pow_di(cos(x), 2); // cos^2
	return -8*(2*JR_pow_di(co2, 2) + 11*co2 + 2) / JR_pow_di(sin(x), 6);
    }
    else
	return ML_NAN;
}
...
void dpsifn(double x, int n, int kode, int m, double *ans, int *nz, int *ierr)
{
	...
	*ierr = 1;
    ...
    const double
	r1m5 = jags_d1mach(5),       // = M_LOG10_2 = log10(2) = 0.30103..
	r1m4 = jags_d1mach(4) * 0.5, // = DBL_EPSILON * 0.5 = 2^-53 = 1.110223e-16
	wdtol = fmax2(r1m4, 0.5e-18); /* = 2^-53 = 1.11e-16 */

    /* elim = approximate exponential over and underflow limit */
    double elim = 2.302 * (nx * r1m5 - 3.0);/* = 700.6174... */
    const double
	rln = fmin2(r1m5 * jags_i1mach(14), 18.06);// = 0.30103 * 53 = 15.95.. ~= #{decimals}
